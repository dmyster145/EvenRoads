...
test("parsePerfLog extracts minSend from bridge stats when present", () => {
  const log = [
    "[HoppyRoads][Perf][Bridge] setupPage=100.0ms",
    "[HoppyRoads][Perf][Bridge] sends=10 avgSend=120.0ms maxSend=200.0ms minSend=80.0ms avgQueue=1.0ms maxQueue=5.0ms coalesced=0 skippedSame=0 droppedLowPri=0 dropRecentInputTick=0 failed=0",
  ].join("\n");
...
test("parsePerfLog handles logs without minSend field (backward compat)", () => {
  const log = [
    "[HoppyRoads][Perf][Bridge] setupPage=100.0ms",
    "[HoppyRoads][Perf][Bridge] sends=5 avgSend=130.0ms maxSend=180.0ms avgQueue=2.0ms maxQueue=10.0ms coalesced=0 skippedSame=0 droppedLowPri=0 dropRecentInputTick=0 failed=0",
  ].join("\n");
...
test("bridge_max_send_high fires when maxSendMs > 250", () => {
  const log = [
    "[HoppyRoads][Perf][Bridge] setupPage=100.0ms",
    "[HoppyRoads][Perf][Bridge] sends=10 avgSend=130.0ms maxSend=300.0ms minSend=90.0ms avgQueue=1.0ms maxQueue=5.0ms coalesced=0 skippedSame=0 droppedLowPri=0 dropRecentInputTick=0 failed=0",
  ].join("\n");
...
test("bridge_send_spike_spread fires when max/min ratio > 2.5", () => {
  const log = [
    "[HoppyRoads][Perf][Bridge] setupPage=100.0ms",
    "[HoppyRoads][Perf][Bridge] sends=10 avgSend=130.0ms maxSend=300.0ms minSend=80.0ms avgQueue=1.0ms maxQueue=5.0ms coalesced=0 skippedSame=0 droppedLowPri=0 dropRecentInputTick=0 failed=0",
  ].join("\n");
...
test("bridge_send_spike_spread does not fire when spread is narrow", () => {
  const log = [
    "[HoppyRoads][Perf][Bridge] setupPage=100.0ms",
    "[HoppyRoads][Perf][Bridge] sends=10 avgSend=110.0ms maxSend=130.0ms minSend=90.0ms avgQueue=1.0ms maxQueue=5.0ms coalesced=0 skippedSame=0 droppedLowPri=0 dropRecentInputTick=0 failed=0",
  ].join("\n");
